    ...
    protected void onFinishInflate() {
        ...
        if (Settings.System.getInt(mContext.getContentResolver(), "blur_style", 0) == 1) {
            mHandler.post(() -> {
                ...
                int statusBarHeight = r.getDimensionPixelSize(
                           r.getIdentifier("status_bar_height", "dimen", "android"));
                int navigationBarHeight = r.getDimensionPixelSize(
                           r.getIdentifier("navigation_bar_height", "dimen", "android"));

                lp.topMargin = statusBarHeight - (statusBarHeight*2);
                lp.bottomMargin = navigationBarHeight - (navigationBarHeight*2);
                setLayoutParams(lp);
                invalidate();
            });
        }
